...
// Solo se usan si el backend falla Y el caché IDB está vacío.
...
const Entrada: React.FC = () => {
    ...
    // 1. Intenta caché IDB (disponible si ya hubo login previo con conexión).
    // 2. Si el caché está vacío, va al backend y persiste en IDB para la próxima vez.
    // Al mapear la respuesta del backend se agrega el icono (campo solo de frontend).
        ...
        const cargarRefData = async () => {
            ...
            try {
                ...
                if (ubicacionesCache && ubicacionesCache.length > 0) {
                    ...
                    return // caché disponible, no necesitamos ir al backend
                }

                // Caché vacío → fetch al backend
                const [tiposRed, ubicacionesRed] = await Promise.all([
                    ingresoService.getTiposVehiculo(),
                    ingresoService.getUbicaciones(),
                ])

                // Mapear TipoVehiculo del backend → TipoVehiculoRef (agrega icono frontend)
                ...
                if (tiposConIcono.length > 0) {
                    setTipos(tiposConIcono)
                    setSelectedTipo(tiposConIcono[0].id)
                }

                // Persistir en IDB (con iconos ya resueltos) para acceso offline futuro
                await refDataService.syncToIndexedDB()

            } catch (err) {
                console.error('[Entrada] Error cargando datos de referencia:', err)
                // Último recurso: fallbacks hardcodeados (ya están en el estado inicial)
            } finally {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="material-symbols-outlined" style={{ fontSize: '22px' }}>
                                                {tipo.icono ?? iconoParaTipo(tipo.nombre)}
                                            </span>
                ...
                {ticketData && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: '#f0f4f8', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#fff', borderBottom: '1px solid #e2e8f0' }}>
                            ...
                            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#334155', padding: '4px', outline: 'none' }}>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto', padding: '16px', paddingBottom: '100px' }}>
                            <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                                <div style={{ textAlign: 'center', padding: '20px 20px 16px' }}>
                                    <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e0ecff', color: '#137fec', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px', fontWeight: 700, fontSize: '18px' }}>P</div>
                                ...
                                <div style={{ margin: '0 20px 16px', background: '#f8fafc', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
                                    <div style={{ fontSize: '12px', fontWeight: 800, color: '#137fec', letterSpacing: '2px', marginBottom: '12px' }}>PARKING TICKET</div>
                                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                                        <QRCode value={JSON.stringify({ id: ticketData.idIngreso, placa: ticketData.placa, ubicacion: ticketData.ubicacion, entrada: ticketData.fechaHoraIngreso })} size={140} level="M" />
                                    </div>
                                ...
                                <div style={{ margin: '0 16px 16px', borderTop: '2px dashed #e2e8f0' }} />
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: '10px', padding: '12px 16px', background: '#fff', borderTop: '1px solid #e2e8f0' }}>
                            <button
                                style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', background: 'linear-gradient(135deg, #137fec 0%, #0b63be 100%)', color: '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: '0 4px 14px rgba(19, 127, 236, 0.3)', outline: 'none', transition: 'all 0.2s' }}
                            ...
                            >
                                <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>download</span>Descargar PDF
                            </button>
                            <button
                                style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff', color: '#334155', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', outline: 'none', transition: 'all 0.2s' }}
